...
logger = logging.getLogger('dissonance.module')
_sentinel = object()


class Modules(object):
    """
        Holds all the loaded modules for a given Dissonance instance.
    """
    _reserved_module_names = {'dissonance', 'web', 'adapter'}
    ...
    def _import_module(self, name, module_instance):
        ...
        try:
            ...
            return import_first_matching_module(mod_name=name,
                                                matches=['modules.%s', 'dissonance_modules.%s'],
    ...
    def load_all(self, modules=None):
        """
            Loads all the modules defined in Dissonance's configuration
        """
    ...
    def iter_module_names_and_opts(self, modules=None):
        if modules is None:
            if 'modules' in self.client._opts:
                modules = self.client._opts['modules']

            else:
                modules = getattr(self.client.config, 'modules', {})
        ...
        for module_name, opts in modules.items():
    ...
    def unload_all(self):
        """
            Unloads all modules.
        """
        for module in list(self._module_dict.keys()):
            self.unload(module)


class Module(object):
    """
        The brains of a Dissonance module.
    ...
    """
    STOP = object()
    __slots__ = ['client', 'opts', '_name', 'author', 'description', '_module_name',
    ...
    def _validate_opts(self):
        ...
        for definition in self.opts._opt_definitions.values():
    ...
    def _handle_message(self, message):
        ...
        if message.tokens:
            ...
            for _, regex, responder, f in self._regex_listeners:
                if responder and not message.targeting_client:
                    continue
    ...
    def _load_data(self):
        return self.client._get_module_data(self)
    ...
    def opt(self, *args, **kwargs):
        ...
        tell Dissonance to automatically throw an exception when the module is trying to be loaded, providing the user
    ...
    def match(self, regex, flags=0, priority=0):
        """
            Decorator that registers a function that will be called when Dissonance sees a message that matches regex.
        """
    ...
    def respond(self, regex, flags=re.I, priority=0):
        """
            Decorator that registers a function that will be called when any message directed at Dissonance matches the
            regex.
        """
        regex = re.compile(regex, flags)
    ...
    def listen(self, priority=0):
        """
            Decorator that registers a function that will be called any time Dissonance sees a message.
        """
