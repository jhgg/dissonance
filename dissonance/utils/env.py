...
class EnvFallbackDict(object):
    """
        ...
        look in "DISSONANCE_TEST_FOO" and return that value. If it's not inside the dict, or environ, it will raise
    ...
    def environ_key(self, key):
        if not self._module_name:
            return ('dissonance_%s' % key).upper()

        return ('dissonance_%s_%s' % (self._module_name, key)).upper()
